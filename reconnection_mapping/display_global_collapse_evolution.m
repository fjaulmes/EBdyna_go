...
% figure(1)
% plot(1:Nradial,P_initial_profile/P0,'b--','LineWidth',3);
% % plot(rho_scale,P_alphas_ini/P0,'g--','LineWidth',3);
% grid on;
% hold on;
% plot(1:Nradial,P_final_profile/P0,'r','LineWidth',3);
% 
psi_pos_q1=interp1(q_initial_profile,1:Nradial,1)

[eps_min inversion_radius_pos ]=min(abs(P_initial_profile(1:round(psi_pos_q1))-P_final_profile(1:round(psi_pos_q1))));
...
grid on;
hold on;
plot(radial_r_value_flux,q_initial,'b--','LineWidth',3);
% xlabel('r (m)');
ylabel('q');
%plot(pos_Bstar_final,q_final_interp,'r');
plot(radial_r_value_flux,q_final,'r','LineWidth',3);
plot(radial_r_value_flux,q_final_profile_diff,'--','Color',[1 0.6 0.3],'LineWidth',3);
ylim([0.8 2.5]);
xlim([0 0.55])
...
subplot(2,1,2);
set(gca,'FontSize',16);
plot(radial_r_value_flux,psi_star_initial,'b--','LineWidth',3);
grid on;
hold on;
plot(radial_r_value_flux,psi_star_final,'r','LineWidth',3);
% xlabel('r (m)');
ylabel('\Psi_* (T.m^{-2})');
% legend('\psi_{*-}','\psi_{*+}');
