...
for (r=1:NR)
    ...
    z1_axis(r,1)=a1*((rv2-rx^2)*cos(alpha)+rx*(rx-sign(ksi0)*r_value*cos(omega_value))*sin(alpha));
    z2_axis(r,1)=a2*(rx^2-rv2);
    omega=Nomega;
    omega_value=scale_omega(omega);
    z1_axis(r,2)=a1*((rv2-rx^2)*cos(alpha)+rx*(rx-sign(ksi0)*r_value*cos(omega_value))*sin(alpha));
