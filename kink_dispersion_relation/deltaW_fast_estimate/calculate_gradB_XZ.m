...
for (x=3:sizeX-2)
    for (z=3:sizeZ-2)
        gradB_X(x,z)=(1/12)*(-Btot_ini_XZ_map(x+2,z)+Btot_ini_XZ_map(x-2,z))+(2/3)*(Btot_ini_XZ_map(x+1,z)-Btot_ini_XZ_map(x-1,z));
        gradB_Z(x,z)=(1/12)*(-Btot_ini_XZ_map(x,z+2)+Btot_ini_XZ_map(x,z-2))+(2/3)*(Btot_ini_XZ_map(x,z+1)-Btot_ini_XZ_map(x,z-1));
    end
end
gradB_X=gradB_X/DX;
gradB_Z=gradB_Z/DX;
