...
TPRECISE=1;
TIME_STAMP_PRECISION=16;
DELTA_TIME=(1e-9)/TPRECISE;
h=DELTA_TIME;
NB_TIME_STEPS=round(0.2*(tau_cr/FAST_SAWTOOTH)/DELTA_TIME)
...
EVOLVE_VEXB=1;
USE_LAP_PSI=0;
SAVENAME=strcat('few_particles_eq_G200415_rot35_h',num2str(TPRECISE))
...
alphas_Ekin=1*[ 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6 6]*1e3;  % keV
...
MIN_ALPHAS_EKIN=0.01*min(alphas_Ekin)

alphas_vpll=(ones(Nalphas_simulated,1)*0.6);

alphas_vpll=alphas_vpll.*sqrt(alphas_Ekin*2*eV/mHe);
alphas_vpll(33:64)=1.5*alphas_vpll(33:64);
...
OMEGAPHI=0.35*1e5
alphas_momentum=alphas_Ekin*0+OMEGAPHI;

% vZ_centrifug=-((1/ZHe)*(mHe/eV)*alphas_Rpos*OMEGAPHI^2).*bphi./alphas_Bfield;
% vphi_centrifug=((1/ZHe)*(mHe/eV)*alphas_Rpos*OMEGAPHI^2).*bZ./alphas_Bfield;
...
%         estimate_half_time_step_positions_eq_G;
        update_GT_3D_eq_next;
        alphas_psi_value_corr=alphas_psi_value;

%         v_X_step=v_X+(v_X_next-v_X_prev)/4;
%         v_Z_step=v_Z+(v_Z_next-v_Z_prev)/4;
%         v_phi_step=v_phi+(v_phi_next-v_phi_prev)/4;
%         v_X_step=0.5*(v_X+v_X_next);
%         v_Z_step=0.5*(v_Z_tot+v_Z_next);
%         v_phi_step=0.5*(v_phi_tot+v_phi_next);
...
%         v_phi=0.5*(v_phi+v_phi_tilde);
         v_phi_step_recalc=(alphas_pphi0+(ZHe)*alphas_psi_value_corr)./((mHe/eV)*(alphas_pos_x+R0));
...
%         v_phi_step=(ZHe)*alphas_psi_value_corr./((mHe/eV)*(alphas_pos_x+R0));
%         v_phi=calc_vnext_value(v_phi_prev_prev,v_phi_prev,v_phi_step);
...
%         v_phi_prev=v_phi;
        ...
        vphi_raw_output(time_stamp,:)=v_phi;
        vphi_next_output(time_stamp,:)=v_phi_next;
