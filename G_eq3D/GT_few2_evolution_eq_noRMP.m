...
if POINCARE_SIMULATION==0
    tau_cr=2e-4;            % Simulation time (typical crash time)
    TPRECISE=8;             % Precision of timesteps in a.u. larger means smaller time steps
    TIME_STAMP_PRECISION=1*TPRECISE; % stamping of time stamps
    DELTA_TIME=(1e-9)/TPRECISE; % size of time stamps (1e-9 timestamp)
    RECORD_TIME_STAMP=1000
else
    tau_cr=0.2;                % Long simulation necessary for field line tracing
    TPRECISE=2;                % Precision of timesteps in a.u. larger means smaller time steps
    TIME_STAMP_PRECISION=2*TPRECISE; % stamping of time steps
    DELTA_TIME=(1e-6)/TPRECISE;      % size of time stamps (1e-6 timestamp)
...
ECOEF=0.0/TPRECISE
PPHI_CORR_FACTOR=0.0/TPRECISE
PPHI_CORR_INTEG_FACTOR=0.0
...
DISPLAY_OUTPUTS=0;
if POINCARE_SIMULATION==0
    SAVENAME=strcat('few_particles_eq_G090216_h',num2str(TPRECISE))
else
    SAVENAME=strcat('few_particles_eq_poincarre_noRMP_0p04eV_G110215_h',num2str(TPRECISE))
end
...
if DISPLAY_OUTPUTS==0
    ...
    disp('**************************************************************');
% else
%     filename=strcat(DATA_FOLDER,'tokamak_PR_map.mat'); % Load PR map
%     load(filename);
%     XXsmall=XX_small; 
%     ZZsmall=ZZ_small;
...
if POINCARE_SIMULATION==0
    ...
    % ENERGY AND VELOCITY
    alphas_Ekin=20*[ 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8]*1e3;  % keV
    ...
    alphas_Ekin=0.005*[ 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8 8];  % eV
    alphas_Ekin=repmat(alphas_Ekin',4,1);
    alphas_vpll=(ones(Nalphas_simulated,1)*1.0);
...
if APPLY_RMP_FIELD==1
    ...
    [IL3D_1 IL3D_2 IL3D_3 IL3D_4 IL3D_5 IL3D_6 IL3D_7 IL3D_8 slopex slopey slopez] = ...
        build_3Dinterp_indexarrays(scale_phi, scale_theta, scale_psi, DPHI,DTHETA,2,alphas_pos_phi_wrapped, alphas_theta, alphas_psi);
...
for time_step=1:NB_TIME_STEPS
    ...
    if (mod(time_step,TIME_STAMP_PRECISION)==0)
        ...
        pphi_outputG(:,time_stamp,:)=(mHe/eV)*(alphas_pos_x+R0).*v_phi_step-(ZHe)*alphas_psi_value_corr;
    ...
    if (SAVE_DATA_FILE==1) && (mod(time_step,RECORD_TIME_STAMP)==0)
        ...
        save (SAVENAME ,'alphas_Ekin','alphas_pphi0','Bfield_outputG','Xpos_outputG','Zpos_outputG','phipos_outputG','theta_outputG',...
            'psi_value_outputG','vparallel_outputG','Ekin_outputG','Ekin_half_outputG','Eperp_outputG','rhoL_outputG','psipos_outputG',...
            'vphi_outputG','pphi_outputG','time_scale','alphas_ejected');
        disp('------------------------------------------------')
    end
