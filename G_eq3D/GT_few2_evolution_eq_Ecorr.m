...
ECOEF=0.0/TPRECISE
%ECOEF_P=1.0/TPRECISE
%ECOEF_INTEG=0.1
% PPHI_CORR_FACTOR=0.032/TPRECISE
% PPHI_CORR_INTEG_FACTOR=0.05
PPHI_CORR_FACTOR=0.0/TPRECISE
PPHI_CORR_INTEG_FACTOR=0.0


TIME_STAMP_PRECISION=2*TPRECISE;
DELTA_TIME=(1e-9)/TPRECISE;
h=DELTA_TIME;
NB_TIME_STEPS=round(0.1*(tau_cr/FAST_SAWTOOTH)/DELTA_TIME)
...
SAVE_DATA_FILE=1;
DISPLAY_OUTPUTS=0;
SAVENAME=strcat('few_particles_eq_precise_160keV_G080215_ECOEF0p0_pphi0p0_h',num2str(TPRECISE))

%time step corrrection to Fmirror map
% Fmirror_XZ_map=h*Fmirror_XZ_map;
...
alphas_vpll=(ones(Nalphas_simulated,1)*0.15);

alphas_vpll=alphas_vpll.*sqrt(alphas_Ekin*2*eV/mHe);
alphas_vpll(33:64)=4*alphas_vpll(33:64);
...
Ekin_output2=zeros(NB_TIME_STAMPS,Nalphas_simulated);
...
alphas_Ekin=0.5*(mHe/eV)*vtot_recalc_sq;
...
v_phi_step_recalc=v0_phi;
v_phi_corr_integ=v0_phi*0;
vtot_integ=v0_phi*0;
delta_Ekin=v0_phi*0;
...
%         v_phi_prev=v_phi;
		...
		Ekin_output2(time_stamp,:)=0.5*(mHe/eV)*(v_X.^2+v_Z.^2+v_phi.^2);
        vphi_output(time_stamp,:)=v_phi_step;
        pphi_output(time_stamp,:)=(mHe/eV)*(alphas_pos_x+R0).*v_phi_step-(ZHe)*alphas_psi_value_corr;
    ...
    if (SAVE_DATA_FILE==1) && (mod(time_step,1000)==0)
	    if time_step==1000
			alphas_pphi0=pphi_output(time_stamp,:);
			alphas_pphi0=alphas_pphi0';
			alphas_Ekin=0.5*(mHe/eV)*(v_X.^2+v_Z.^2+v_phi.^2);
		end
...
if SAVE_DATA_FILE==1
     save (SAVENAME ,'alphas_Ekin','alphas_pphi0','Bfield_outputG','Xpos_outputG','Zpos_outputG','phipos_outputG','theta_outputG',...
         'psi_value_outputG','vparallel_outputG','Ekin_outputG','Ekin_output2G','Eperp_outputG','rhoL_outputG','psipos_outputG',...
