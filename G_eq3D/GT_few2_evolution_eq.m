...
REINIT_ALL_MAPS=0           | ~exist(filename, 'file');
REINIT_SIMULATION_DATA=1    | REINIT_ALL_MAPS;
%% Reinitialization
% Maps generation
...
if POINCARE_SIMULATION==0
    ...
    tau_cr=2e-4;                % Long simulation necessary for field line tracing
    TPRECISE=1;                  % Precision of timesteps in a.u. larger means smaller time steps
...
if POINCARE_SIMULATION==0
    ...
    SAVENAME=strcat('few_particles_eq_ssimple_vphi_noRMP_35keV_p0p1_G',datestr(now,'DDmmYY'),'_h',num2str(TPRECISE))
end
...
if POINCARE_SIMULATION==0
    ...
    alphas_pos_z(1:Nalphas_fourth+1)=0.33*((0:Nalphas_fourth)/Nalphas_fourth);
    ...
    alphas_pos_x(round(0.5*Nalphas_simulated)+1:Nalphas_simulated)=0.88*((0:Nalphas_half)/Nalphas_half-0.5)
    ...
    alphas_Ekin=(1e3)*repmat(35,Nalphas_simulated,1);  % keV
	alphas_vtot=sqrt(alphas_Ekin*2*eV/mHe); %Absolute value for speed |v|
...
for time_step=1:NB_TIME_STEPS
    ...
    if (mod(time_step,TIME_STAMP_PRECISION)==0)
        ...
        v_Z_step=(0.5*v_Z+0.5*v_Z_prev);
        v_phi_step=(0.5*v_phi+0.5*v_phi_prev);
%         v_phi_step=calc_vstep_value(v_phi_prev_prev,v_phi_prev,v_phi);
        v_X=v_X_prev;
        v_Z=v_Z_prev;
        v_phi=v_phi_prev;
