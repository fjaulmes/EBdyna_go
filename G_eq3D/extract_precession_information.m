...
r_output_pop=interp1(1:length(radial_r_value_flux),radial_r_value_flux,r_output_pop,'*linear');
...
clear vD_phi_XZ_map vD_Z_XZ_map psi_norm_XZsmall_map q_initial_XZsmall_map gradZ_theta_map theta_XZsmall_map
...
der_theta_pi_pop=zeros(Nalphas_simulated,1);
der_theta_pi_pop_prev=zeros(Nalphas_simulated,1);
der_theta_pi_pop_next=zeros(Nalphas_simulated,1);
...
for number=1:Nalphas_simulated
	...
	der_theta_pi_pop_prev(number)=theta_output_pop(min(HFS_theta_pos,end_ts),number)-theta_output_pop(max(HFS_theta_pos-5,1),number);
	der_theta_pi_pop_next(number)=theta_output_pop(min(HFS_theta_pos+5,end_ts),number)-theta_output_pop(max(HFS_theta_pos,1),number);
...
HFS_crossing=(sign(der_theta_pi_pop_prev).*sign(der_theta_pi_pop_next)==1);
HFS_crossing=(minX_value_pop<=X_axis).*HFS_crossing;
% two conditions need to be verified
HFS_crossing=(HFS_crossing.*((delta_theta_pi_pop<=(0.012-0.011*sqrt(r_avg/max(r_avg))))));
...
STAGNATION_POP=((~sign_change_pop).*(~HFS_crossing).*(minX_value_pop>=X_axis).*sigma_LFS)+((~sign_change_pop).*(~sigma_LFS).*(HFS_crossing));
...
calculate_precession_distribution

save_distribution_precession_data
