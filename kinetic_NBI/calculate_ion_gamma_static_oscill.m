    ...
    if (alphas_time_orbit(n)>0)
        ...
        alphas_vD_X=alphas_vD_X.*cos(OMEGA_KINK_REAL*time_scale(TIMEINI:alphas_time_orbit(n)))';
        alphas_vD_Z=alphas_vD_Z.*cos(OMEGA_KINK_REAL*time_scale(TIMEINI:alphas_time_orbit(n)))';
        alphas_vD_phi=alphas_vD_phi.*cos(OMEGA_KINK_REAL*time_scale(TIMEINI:alphas_time_orbit(n)))';
        ...
        % rotation of the mode !!!
        alphas_E_X=alphas_E_X.*sin(OMEGA_KINK_REAL*time_scale(TIMEINI:alphas_time_orbit(n)))';
        alphas_E_Z=alphas_E_Z.*sin(OMEGA_KINK_REAL*time_scale(TIMEINI:alphas_time_orbit(n)))';
        alphas_E_phi=alphas_E_phi.*sin(OMEGA_KINK_REAL*time_scale(TIMEINI:alphas_time_orbit(n)))';
        alphas_dB=alphas_dB.*cos(OMEGA_KINK_REAL*time_scale(TIMEINI:alphas_time_orbit(n)))';
        
%          alphas_dB(INNER_PART)=interp2(theta_scale,1:size_r,dBtot_map_theta',alphas_theta(INNER_PART),alphas_psi(INNER_PART));
%          alphas_dB(OUTER_PART)=0;
...
%         POWER_ION=NBSPLITS*transp_weight*eV*mean(alphas_mm_t.*alphas_dB);
    ...
    if (abs(GAMMA_ION)>550) %&& (abs(GAMMA_ION)>90)
        n
        %
        figure(2)
        hold on
