...
build_vD_map_frame_rank;
save (strcat('vD_map_frame_rank',num2str(FRAME_NUMBER),'.mat'),'vD_X_PR_map_phi','vD_Z_PR_map_phi','vD_phi_PR_map_phi','ikink_kinetic_energy_phi');
...
for PROCESS_NUMBER=1:2
    ...
    % co current NBI means counter field destabilizing passing ions
...
%     clear Btot_ini
    ...
    load(strcat('vD_map_frame_rank',num2str(FRAME_NUMBER),'.mat'));
    ...
    %alphas_pos_phi_wrapped=zeros(length(PART_POP),1);
    %alphas_pos_x_t=zeros(length(PART_POP),1);
    %alphas_pos_z_t=zeros(length(PART_POP),1);
    %alphas_theta=zeros(length(PART_POP),1);
    %alphas_psi=zeros(length(PART_POP),1);
    %alphas_vD_X=zeros(length(PART_POP),1);
    %alphas_vD_Z=zeros(length(PART_POP),1);
    %alphas_vD_phi=zeros(length(PART_POP),1);
    %alphas_E_X=zeros(length(PART_POP),1);
    %alphas_E_Z=zeros(length(PART_POP),1);
    %alphas_E_phi=zeros(length(PART_POP),1);
    
    %IL3D_1=zeros(length(PART_POP),1);
    %IL3D_2=zeros(length(PART_POP),1);
    %IL3D_3=zeros(length(PART_POP),1);
    %IL3D_4=zeros(length(PART_POP),1);
    %IL3D_5=zeros(length(PART_POP),1);
    %IL3D_6=zeros(length(PART_POP),1);
    %IL3D_7=zeros(length(PART_POP),1);
    %IL3D_8=zeros(length(PART_POP),1);
    %slopex=zeros(length(PART_POP),1);
    %slopey=zeros(length(PART_POP),1);
    %slopez=zeros(length(PART_POP),1);
    ...
    save (strcat('gamma_ions_co',num2str(PROCESS_NUMBER),'_f',num2str(FRAME_NUMBER),'.mat'),'ions_gamma','ions_deltaE',...
