...
%         dot_product_vD=alphas_E_X.*alphas_vD_X+alphas_E_Z.*alphas_vD_Z+alphas_E_phi.*alphas_vD_phi;
        ...
        GAMMA_ION_VD=-0.5*POWER_ION_VD/(WMHD_KINK)/NB_LOOPS;
        GAMMA_ION_MU=-0.5*POWER_ION_MU/(WMHD_KINK)/NB_LOOPS;
        
        % the mu part should average to 0
        GAMMA_ION=-0.5*POWER_ION_VD/(WMHD_KINK)/NB_LOOPS;
...
if (abs(ions_gamma(n))>600) %&& (abs(GAMMA_ION)>90)
    n
    %
    figure(2)
    hold on
