...
%         poloidal_hist_ini(eb,pb,:,:) = hist2d ([pos_X_gc(PART_POP)+R0 pos_Z_gc(PART_POP)], R_BINS, Z_BINS);
        poloidal_hist_ini(eb,pb,:,:) = hist2d ([alphas_pos_x(PART_POP)+R0 alphas_pos_z(PART_POP)], R_BINS, Z_BINS);
...
load('W40_fc1p6h2_16000_all.mat');
...
figure(1)
contourf(R_values,Z_values,(squeeze(sum(dist_EpitchRZ_end(1,:,:,:),2))'),8);axis xy;hold on
