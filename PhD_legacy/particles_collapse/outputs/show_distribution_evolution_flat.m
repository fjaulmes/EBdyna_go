...
% 
load('initialG_2200keV_flat_pre_collapse.mat');
...
% load('flat2000keV_collapse_Glisa_fc1h2_G070613.mat');
load('flat2200keV_collapse_Glocal_fc1h2_G190813.mat');
% ENERGY_POPULATION_0=find((~alphas_ejected).*(alphas_kappa>=1).*(alphas_kappa<=10));
ENERGY_POPULATION_0=find((~alphas_ejected).*(psi_pos0<=psi_core).*COUNTER_PASSING_POP);
...
plot(Dp_phi_scale,NDpphi/max(NDpphi),'b','LineWidth',2)
% plot(Dp_phi_scale,NDpphi_co/max(NDpphi_co),'r--','LineWidth',2)
xlabel('\Delta p_\phi')

EVALUATED_POPULATION=length(NDpphi)
