...
PART_POP=find((alphas_psi<psi_pos_q1).*ALL_TRAPPED_POP);
...
R_BIN_SIZE=8*1e3;
R_BINS=(-R_BIN_SIZE:R_BIN_SIZE:112*1e3);
...
mHist = hist2d ([alphas_Ekin(PART_POP) alphas_pitch(PART_POP)], R_BINS, PITCH_BINS);
...
hold on;
set(gca,'fontsize',22)
contourf ( r_values,pitch_values, mHist'/max(max(mHist)),(0 :0.05:1));
...
mHistpost = hist2d ([alphas_Ekin(PART_POP) alphas_pitch(PART_POP)], R_BINS, PITCH_BINS);
% pcolor (r_values, pitch_values, mHist);
% shading faceted; 
...
title('initial NBI 60 keV (inside q=1)')
...
contourf ( r_values,pitch_values, mHistpost'/max(max(mHist)),(0 :0.05:1));
% plot([-1 1],[X_axis X_axis],'k--','Linewidth',4)
colorbar
...
% mHistpost(1,1)=0;
% 
% contourf ( pitch_values, r_values,mHistpost-mHist,(-100 :5:100));
% %plot2Dhist(mHist,PITCH_BINS,R_BINS,   'pitch',  'r (m)','Nb part ini'); 
% axis  ij
% xlabel('pitch')
% ylabel('Ekin')
