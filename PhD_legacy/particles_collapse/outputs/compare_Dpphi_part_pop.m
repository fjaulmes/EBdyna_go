...
POSITION_RANGE=(alphas_psi0<=psi_core);

load('alphas_collapse_Glisa_fc1h2_G290713.mat');
pphi_end=alphas_pphi0;
alphas_pos_psi=interp2(scale_X,scale_Z,psi_norm_XZsmall_map',alphas_pos_x,alphas_pos_z,'*linear');
EKIN_INF=40
EKIN_SUP=400
POPULATION_RANGE=(alphas_Ekin>EKIN_INF*1e3).*(alphas_Ekin<EKIN_SUP*1e3).*POSITION_RANGE;
...
Dp_phi_scale=(-1.2:Dp_phi_bin_size:1.2);
...
POPULATION_RANGE=(alphas_Ekin>EKIN_SUP*1e3).*POSITION_RANGE;
ENERGY_POPULATION_0=find(POPULATION_RANGE);
ENERGY_POPULATION=find((~alphas_ejected).*POPULATION_RANGE);
