...
% density_correction_factor=(1e-6)*NB_PART_SIM_TRANSP_RATIO*weight_transp_NBI

%%
PRE_COLLAPSE_FILENAME='initial_NBI60keV_Rlab_pre_collapse_all.mat'
POST_COLLAPSE_FILENAME='NBI60keV_Rlab_fc1p6h1p6_all.mat'


load(PRE_COLLAPSE_FILENAME);

load('weight_correction_profileP.mat')
% alphas_weight_radial_profile=alphas_weight_radial_profile1;
...
figure(2)
set(gca,'fontsize',20)
contourf(R_values,Z_values,1e6*EKIN_BIN_SIZE*PICH_BIN_SIZE*(squeeze(sum(sum(dist_EpitchRZ_ini(1:end,:,:,:),2),1))'),12);axis xy;hold on
% contour(scale_X+R0,scale_Z,psi_norm_XZsmall_map','k','linewidth',2)

%%
figure(3)
set(gca,'fontsize',20)
contourf(R_values,Z_values,1e6*EKIN_BIN_SIZE*PICH_BIN_SIZE*((squeeze(sum(sum(dist_EpitchRZ_end(1:end,:,:,:),2),1))')-squeeze(sum(sum(dist_EpitchRZ_ini(1:end,:,:,:),2),1))'),12);axis xy;hold on
% contour(scale_X+R0,sca1e6*density_correction_factor*le_Z,psi_norm_XZsmall_map','k','linewidth',2)
