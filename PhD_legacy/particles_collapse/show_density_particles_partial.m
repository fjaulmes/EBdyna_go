...
load('initialG_80keV_flat_pre_collapse.mat');
alphas_psi0=alphas_psi;
PLOT_COLOR='b';
% load('flat80keV_collapse_Glisa_fc1h2_G020513_record20000.mat')

PARTS=find((alphas_psi0<80));
PARTS=find((alphas_psi0>60).*(alphas_psi0<130));
% PARTS=find((alphas_psi0>100));
...
alphas_pos_x_part=alphas_pos_x(PARTS);
alphas_pos_z_part=alphas_pos_z(PARTS);

alphas_Xrank=alphas_pos_x_part/DX+801;
Nb_part=0;
clear N_volume_up_binned N_volume_down_binned
% counting the number of particles in each bin
for(x=1:N_X_bins)
    X_pos=X_bin_pos(x);
    particles_in_bin_up=find((alphas_Xrank>=X_pos-X_bin_size_half).*(alphas_Xrank<X_pos+X_bin_size_half).*(alphas_pos_z_part>=Z_axis));
    particles_in_bin_down=find((alphas_Xrank>=X_pos-X_bin_size_half).*(alphas_Xrank<X_pos+X_bin_size_half).*(alphas_pos_z_part<Z_axis));
    for(p=1:N_psi_bins)
        psi=psi_bin_pos(p);
        ...
        PARTS_BINNED=find((Z_up_sup_part-alphas_pos_z_part(particles_in_bin_up)>0).*(alphas_pos_z_part(particles_in_bin_up)-Z_up_inf_part>=0));
        ...
        PARTS_BINNED=find((Z_down_sup_part-alphas_pos_z_part(particles_in_bin_down)>0).*(alphas_pos_z_part(particles_in_bin_down)-Z_down_inf_part>=0));
...
hold on;
grid on;
plot(density_x_profile,PLOT_COLOR);axis xy
