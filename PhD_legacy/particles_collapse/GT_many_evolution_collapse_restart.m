...
h=DELTA_TIME;
NB_TIME_STEPS=round(0.025*(tau_cr/FAST_SAWTOOTH)/DELTA_TIME)
...
% INPUTFILE='initialG_2000keV_flat_pre_collapse.mat'
% SAVENAME='flat2000keV_collapse_Glisa_fc1h2_G070613.mat'


INPUTFILE='alphas_hcollapse_Glisa_fc2h2_G181213.mat'
SAVENAME='alphas_collapse_Glisa_fc1h2_G191213.mat'
...
alphas_ejected=zeros(Nalphas_simulated,1)+alphas_ejected;
...
% these perpendicular vectors are used only to 
...
outcast=find(isnan(alphas_vpll));
recast=randi(Nalphas_simulated,size(outcast,1),1);
alphas_vpll(outcast)=alphas_vpll(recast);
alphas_pphi0(outcast)=alphas_pphi0(recast);
...
disp('checking consistency of pphi0:');
disp(mean(abs(alphas_pphi0-((mHe/eV)*alphas_Rpos.*v_phi-(ZHe)*alphas_psi_value_corr))));
...
disp('checking consistency of Etot:');
disp(mean(abs(alphas_Etot-(alphas_Ekin0+ZHe*alphas_Epot))));
...
%         v_phi_step=(1.5*v_phi-0.5*v_phi_prev);
    ...
    if (SAVE_DATA_FILE==1) && (mod(time_step,2000)==0)
        toc
        tic
        ...
        % filename=strcat('flat80keV_collapse_Glisa_fc1h2_G280413record_',num2str(time_step),'.mat');
        save(SAVENAME,'time','alphas_pos_x','alphas_pos_z','alphas_pos_phi','alphas_omega',...
            'v_X','v_Z','v_phi','alphas_vpll','alphas_mm','alphas_vE_sq','alphas_psi','alphas_pphi0',...
            'alphas_psi_value_corr','alphas_Ekin','alphas_Etot','alphas_ejected','alphas_grad_Phi');
        %         filename=strcat('alphas_record_few_coll',num2str(time_step),'.mat');
        %         save(filename,'time','alphas_pos_x','alphas_pos_z','alphas_pos_phi','alphas_vpll');
        disp('------------------------------------------------')
    end
...
if SAVE_DATA_FILE==1
    save (SAVENAME,'time','pos_X_gc','pos_Z_gc','alphas_pos_x','alphas_pos_z',...
