...
if(omega_cont13==1)    
    ...
    psi_limit13=abs(interp1(scale_r(1:NR),psi_star_3(1:NR,1),r_nu_cont13,'cubic'));
else    
    for(r=2:pos_psi_rx_round+1)
        Dr_value=scale_r(r+1)-scale_r(r-1);
        der_psi(r)=-(psi_star_3(r+1,2)-psi_star_3(r-1,2))/Dr_value;
    end
    psi_limit13=abs(interp1(scale_r(1:NR),psi_star_3(1:NR,2),r_nu_cont13,'cubic'));
end
...
deriv_psi_limit13=abs(interp1(scale_r(1:NR-1),der_psi(1:NR-1),r_nu_cont13,'cubic'));
deriv_psi_limit13_initial=abs(interp1(scale_r(1:NR-1),dPsih(1:NR-1),r_nu_cont13_initial,'cubic'));
...
%deriv_psi_limit13=abs(interp1(scale_r(1:pos_psi_rx+1),der_psi(1:pos_psi_rx+1),rx,'cubic'));
%deriv_psi_limit13_initial=abs(interp1(scale_r(1:NR),dPsih(1:NR),rx,'cubic'));
