...
%     Ekin_bins_lim_max=max(0.5*(mHe/eV)*(vperp_bins_lim.^2))
...
%%
    ...
    for (vperp_pos=VPERP0_BIN_POS:VPERP0_MAX_BIN_POS)
        ...
        vpll=vA;
        integ1_Ekin=integ1_Ekin+((wTAE)*(eV/mHe)*(vpll^2+0.5*vperp^2)*interp1(vperp_bins,dFvperp1,vperp)*SPEED_BIN_SIZE);
        integ1=integ1+((wTAE)*(eV/mHe)*(vpll^2+0.5*vperp^2)*interp1(vperp_bins,dFvperp1,vperp)-...
            (n/ZHe)*(vpll^2+0.5*vperp^2)*vperp*interp2(vperp_bins,1:length(psi_bins),dFpsi1',vperp,psi_pos_TAE))*SPEED_BIN_SIZE;
        vpll=vA/3;
        integ2=integ2+((wTAE)*(eV/mHe)*(vpll^2+0.5*vperp^2)*interp1(vperp_bins,dFvperp2,vperp)-...
            (n/ZHe)*(vpll^2+0.5*vperp^2)*vperp*interp2(vperp_bins,1:length(psi_bins),dFpsi2',vperp,psi_pos_TAE))*SPEED_BIN_SIZE;
        integ2_Ekin=integ2_Ekin+((wTAE)*(eV/mHe)*(vpll^2+0.5*vperp^2)*interp1(vperp_bins,dFvperp2,vperp))*SPEED_BIN_SIZE;
    end
    
    gamma1_Ekin=(1/eV)*(1/Bavg^2)*kpll*vA*2*(pi^2)*mu0*(mHe^2)*R0*(qTAE^3)*integ1_Ekin;
    gamma2_Ekin=(1/eV)*(1/Bavg^2)*kpll*vA*2*(pi^2)*mu0*(mHe^2)*R0*(qTAE^3)*integ2_Ekin;
    gamma_Ekin=gamma1_Ekin+gamma2_Ekin;
    
    gamma1=(1/eV)*(1/Bavg^2)*kpll*vA*2*(pi^2)*mu0*(mHe^2)*R0*(qTAE^3)*integ1;
    gamma2=(1/eV)*(1/Bavg^2)*kpll*vA*2*(pi^2)*mu0*(mHe^2)*R0*(qTAE^3)*integ2;
