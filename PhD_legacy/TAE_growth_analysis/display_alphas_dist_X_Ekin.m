...
PART_POP=find((alphas_Ekin>EKIN_INF).*(alphas_pos_z<0.4).*(alphas_pos_z>-0.4));
% PART_POP=find((alphas_Ekin>EKIN_INF));

PICH_BIN_SIZE=200*1e3;
PITCH_BINS=(300*1e3:PICH_BIN_SIZE:3500*1e3);
...
mHist = hist2d ([alphas_pos_x(PART_POP) alphas_Ekin(PART_POP)], R_BINS, PITCH_BINS);
...
mHistpost = hist2d ([alphas_pos_x(PART_POP) alphas_Ekin(PART_POP)], R_BINS, PITCH_BINS);
% pcolor (r_values, pitch_values, mHist);
% shading faceted; 
