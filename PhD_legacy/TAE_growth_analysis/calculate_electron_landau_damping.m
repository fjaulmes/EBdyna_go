compare_init_kinetic_temperature_profiles
close all;
...
beta_e_profile_initial=2*mu0*Ne_profile.*Te_profile./(Bavg.^2);
vA_radial_profile_initial=Btot_radial_profile./(sqrt(mu0*Ne_profile*mDT));
vthe_radial_profile=sqrt(2*Te_profile/me);
...
% these final profiles come from kinetic calculations 

beta_e_profile_final=2*mu0*Ne_final_profile.*Te_final_profile./(Bavg.^2);
...
vthe_radial_profile=sqrt(2*Te_final_profile/me);
...
% gm_lambdae_final=(0.5*sqrt(pi)).*(1+2*lambda_e_final.^2+2*lambda_e_final.^4).*exp(-lambda_e_final.^2);
% gm_lambdae_3_final=(0.5*sqrt(pi)).*(1+2*(lambda_e_final/3).^2+2*(lambda_e_final/3).^4).*exp(-(lambda_e_final/3).^2);
