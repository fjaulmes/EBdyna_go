...
dA_map_phi=zeros(NB_PHI,NP,size_r_TAE);
dEpot_map_phi=zeros(NB_PHI,NP,size_r_TAE);
...
pphi_TAE=mHe*R0*vA_TAE-ZHe*psiTAE;

filename='../data_tokamak/TAE_data.mat';
disp('****** SAVING TAE DATA in ../data_tokamak/  ****');
save(filename,'kTAE','omega_TAE','nTAE','qTAE','pTAE_inf','pTAE_sup','size_r_TAE','NB_FRAME','NB_PHI','TAE_angle',...
    'vA_TAE','vA3_TAE','psiTAE','pphi_TAE');
nTAE
...
%     save(filename,'BsX_map_phi','BsZ_map_phi','Bsphi_map_phi','psi_map_phi','iA_map_phi');
    save(filename,'BsX_map_phi','BsZ_map_phi','Bsphi_map_phi','A_map_phi','iA_map_phi','dA_map_phi');
%    save(filename,'BsX_map_phi','BsZ_map_phi','Bsphi_map_phi');
    disp('****** SAVING FILE in ../B_maps/  ****');
    
    filename='../E_maps/E0';
    filename=strcat(filename,frame_name,'.mat');   
    save(filename,'Efield_X_map_phi','Efield_Z_map_phi','Epot_map_phi','iEpot_map_phi','dEpot_map_phi');
