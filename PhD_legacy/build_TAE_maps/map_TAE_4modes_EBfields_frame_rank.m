...
for phi_rank=1:NB_PHI
    ...
    map_TAE_4modes_EBfields_phi_rank;
...
%         (-P1.*div_ksi)));
    WK_TAE_phi(phi_rank)=nTAE*0.5*DPHI*DX*DX*sum(sum(Rpos_XZsmall_map.*...
        (mDT*ion_density_XZ_map).*(ksi_dot_XZ_map_frame_rank).^2));
...
%     ksi_amplitude=sqrt((WK_TAE_phi(phi_rank)-Wmag_TAE_phi(phi_rank))/WP_TAE_phi(phi_rank));
    ...
    W_TAE_phi(phi_rank)=Wmag_TAE_phi(phi_rank)+WP_TAE_phi(phi_rank)+WK_TAE_phi(phi_rank);
