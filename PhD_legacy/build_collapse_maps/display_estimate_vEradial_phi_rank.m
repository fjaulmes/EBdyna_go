...
set(gca,'FontSize',22);
hold on
imagesc(scale_X,scale_Z , (vEr_map1.*gradr_psi_map)',[-6000 6000]);
...
xlim([-0.3 0.1])
ylim([-0.5 0.5])
t_h=title('$$v_{Er} (\varphi=0)$$','interpreter','latex')
...
set(gca,'FontSize',22);
hold on
imagesc(scale_X,scale_Z , (vEr_map2.*gradr_psi_map)',[-6000 6000]);
contour(scale_X_small,scale_Z_small,theta_map',15,'k')
contour(scale_X_small,scale_Z_small,psi_norm_map',15,'k')
axis xy
% axis square
...
ylim([-0.5 0.5])
% xlim([-0.35 0.05])
t_h=title('$$v_{Er} (\varphi=\pi)$$','interpreter','latex')
set(t_h,'Interpreter','latex');
colorbar('North')
