...
%         delta_Epot(INNER_PART_GC)=delta_Epot(INNER_PART_GC)+(alphas_Epot_gc(INNER_PART_GC)-alphas_Epot_gc_part_prev(INNER_PART_GC));
       % Kinetic energy evolution
        % the division par Rpos is not useful here
        delta_Epot(INNER_PART_GC)=ZHe*((alphas_Epot_gc(INNER_PART_GC)-alphas_Epot_gc_prev(INNER_PART_GC))+(v_phi_gc(INNER_PART_GC)./bphi_gc(INNER_PART_GC)).*delta_ps(INNER_PART_GC));
        delta_E(INNER_PART_GC)=delta_E(INNER_PART_GC)+delta_Epot(INNER_PART_GC);
        alphas_Etot(INNER_PART_GC)=alphas_Etot(INNER_PART_GC)+delta_Epot(INNER_PART_GC);
