function interp_values=interp2_psi_theta_flc_map(scale_psi,scale_theta,FXZ_map,Dpsi,Dtheta,alphas_pos_psi,alphas_pos_theta,PART_LIST)


% alphas_pos_psi(PART_LIST)=min(alphas_pos_psi(PART_LIST),scale_psi(end));
alphas_pos_theta(PART_LIST)=min(alphas_pos_theta(PART_LIST),scale_theta(end));
% alphas_pos_psi(PART_LIST)=max(alphas_pos_psi(PART_LIST),scale_psi(1));
alphas_pos_theta(PART_LIST)=max(alphas_pos_theta(PART_LIST),scale_theta(1));
    

Xindex_precise=(alphas_pos_psi(PART_LIST)/Dpsi);
Zindex_precise=(alphas_pos_theta(PART_LIST)/Dtheta)+1;
...
EXCEEDING_OMEGA=find(Zindex>(length(scale_theta)-1));
