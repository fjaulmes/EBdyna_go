...
%         delta_Epot(INNER_PART_GC)=delta_Epot(INNER_PART_GC)+(alphas_Epot_gc(INNER_PART_GC)-alphas_Epot_gc_part_prev(INNER_PART_GC));
       % Kinetic energy evolution
        ...
        delta_ps(INNER_PART_GC)=ZHe*((alphas_Epot_gc_prev(INNER_PART_GC))+(v_phi(INNER_PART_GC)).*delta_ps(INNER_PART_GC));
        if USE_DELTAE_TH==1
...
% 		alphas_grad_Phi(INNER_PART)=-(omega_TAE/kTAE)*alphas_grad_psi_star(INNER_PART);
        ...
        alphas_pphi0=(2*alphas_pphi0+alphas_pphi0_prev)/3;
%     else
...
%         INNER_TH_PART=find((abs(delta_ps)>MIN_DIV_DPPHI).*(alphas_psi<=(pTAE_sup-1)).*(alphas_psi>=(pTAE_inf+1)));
        ...
        % delta_ps(INNER_BLOWUP_PART)=(1/MIN_DIV_DPPHI)*delta_E_th(INNER_BLOWUP_PART);
        delta_ps(INNER_BLOWUP_PART)=(omega_TAE/nTAE)*((1-(kTAE/omega_TAE)*v_phi(INNER_BLOWUP_PART)./bphi(INNER_BLOWUP_PART))/MIN_DIV_DPPHI).*delta_ps(INNER_BLOWUP_PART);
...
%         alphas_Ekin=max(alphas_Etot-ZHe*alphas_Epot,0);
       ...
       alphas_Ekin=max(alphas_Etot_th-ZHe*alphas_Epot,0);
