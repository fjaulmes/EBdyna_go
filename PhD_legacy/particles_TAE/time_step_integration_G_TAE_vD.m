...
%     update_G_3D_TAE;
    ...
    if (CALCULATE_DELTAE_POWER==1)
        ...
        % psi star evolution
%         delta_ps(INNER_PART_GC)=(alphas_psi_star_prev(INNER_PART_GC));
...
%         INNER_TH_PART=find((abs(delta_ps)>MIN_DIV_DPPHI).*(alphas_psi<=(pTAE_sup-1)).*(alphas_psi>=(pTAE_inf+1)));
        ...
        % energy evolution
        delta_ps(INNER_PART)=alphas_pphi0(INNER_PART)-alphas_pphi0_prev(INNER_PART);
...
%         delta_Epot=alphas_Epot-alphas_Epot_prev;
        ...
        alphas_Ekin=alphas_Ekin-delta_Ekin_vD;
%         alphas_Ekin=(alphas_Etot-ZHe*alphas_Epot);
%         delta_Ekin=delta_Ekin+alphas_Ekin-alphas_Ekin_prev;
