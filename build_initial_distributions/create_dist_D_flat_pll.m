...
% adjusting the edge to avoid strange numerical artifacts
X_PR_map(:,end+1)=X_PR_map(:,end);
Z_PR_map(:,end+1)=Z_PR_map(:,end);
% X_PR_map(:,end-1)=0.5*(X_PR_map(:,end-1)+X_PR_map(:,end-2));
% Z_PR_map(:,end-1)=0.5*(Z_PR_map(:,end-1)+Z_PR_map(:,end-2));
...
% This can be used to mimick an NBI distribution
MIN_NEGATIVE_PITCH=1.0
DELTA_PITCH=2-(1-MIN_NEGATIVE_PITCH)

for psi_rank=psi_pos_inf:psi_pos_sup
    
    psi_bin_inf=radial_bins_range(psi_rank);
    psi_bin_sup=radial_bins_range(psi_rank+1);
    ...
    if (psi_rank<length(Npart_flux_surface)) && (psi_rank>1)
        Npart_prev=0.5*(Npart_flux_surface(psi_rank-1)+Npart_flux_surface(psi_rank));
        Npart_next=0.5*(Npart_flux_surface(psi_rank)+Npart_flux_surface(psi_rank+1));
    elseif psi_rank==1
        Npart_prev=0.5*(Npart_flux_surface(psi_rank)+Npart_flux_surface(psi_rank));
        Npart_next=0.5*(Npart_flux_surface(psi_rank)+Npart_flux_surface(psi_rank+1));
    elseif psi_rank==length(Npart_flux_surface)
        Npart_prev=Npart_flux_surface(psi_rank-1);
        Npart_next=Npart_flux_surface(psi_rank);
    end
    for(n=1:DNpart)
        psi_vector(n)=my_rand_linear_dist(Npart_prev,Npart_next,psi_bin_inf,psi_bin_sup);
    end
    ...
    plot(0.5+psi_bin_inf:0.5+psi_bin_sup-1,psi_vector_plot(1:end-1))
    ...
    if (n<DNpart)
        ...
        disp('adding the difference in the highest energy bin');
        disp('Adding:');
        added_particles_energy_highest=DNpart-n
        for missing_part=1:added_particles_energy_highest
            Epart_vector(n+missing_part)=my_rand_linear_dist(f_alpha_E_bin_edge_inf(psi_rank,E_rank),f_alpha_E_bin_edge_sup(psi_rank,E_rank),energy_D(E_rank)-0.5*energy_bin_size,energy_D(E_rank)+0.5*energy_bin_size);
        end
        Nalpha_energy_rank(E_rank)=Nalpha_energy_rank(E_rank)+added_particles_energy_highest;
    end
    ...
    alphas_pos_x(Npart_rank_flux_surface_begin:Npart_rank_flux_surface_end)=interp2(theta_scale,1:Nradial+1,X_PR_map',theta_vector,psi_vector,'cubic');
    alphas_pos_z(Npart_rank_flux_surface_begin:Npart_rank_flux_surface_end)=interp2(theta_scale,1:Nradial+1,Z_PR_map',theta_vector,psi_vector,'cubic');
...
OUTSIDE_PART=find(alphas_psi>0);
plot(alphas_pos_x(OUTSIDE_PART),alphas_pos_z(OUTSIDE_PART),'r.')
% now remove all particles directly outisde of simulation domain
% Co-current case
INSIDE_PART=find(alphas_psi<=0);
alphas_pos_x=alphas_pos_x(INSIDE_PART);
alphas_pos_z=alphas_pos_z(INSIDE_PART);
alphas_Ekin=alphas_Ekin(INSIDE_PART);
alphas_vpll=alphas_vpll(INSIDE_PART);
