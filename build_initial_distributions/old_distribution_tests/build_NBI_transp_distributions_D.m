...
NB_PROCESS=16
NB_SPLITS=round(0.5*NB_PROCESS);
...
for PROCESS_NUMBER=1:NB_PROCESS
    ...
    if PROCESS_NUMBER==SPLIT_RATIO
        disp('------- getting back to initial data -------')
        DIM_RESCALE=1.0
    end
    
    d_pos_x=data(INIT_DATA_POS:SPLIT_RATIO:end,1)*0.01+rmaxis-(R0+X_axis)-rgeo;
    ...
    alphas_pos_x=DIM_RESCALE*[d_pos_x ];
    alphas_pos_z=DIM_RESCALE*[d_pos_z ]+Z_axis;
    alphas_Ekin=[d_Ekin];
    alphas_vpll=[d_vpll];
    ...
    save (FILENAME,'alphas_pos_x','alphas_pos_z','alphas_pos_phi','alphas_Ekin','alphas_mm','alphas_vpll','Nalphas_simulated','density_part_ratio','mHe','ZHe');
