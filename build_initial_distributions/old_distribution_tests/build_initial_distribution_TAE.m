...
build_radial_particles_binning_TAE;
...
Nalpha_binned=interp1(1:Nradial, fast_density_profile,radial_bins);
%Normalize the alpha density
Nalpha_max=max(Nalpha_binned);
Nalpha_binned_norm=Nalpha_binned/Nalpha_max;
Nalpha_binned_norm(psi_pos_sup)=0.5*(Nalpha_binned_norm(psi_pos_sup+1)+Nalpha_binned_norm(psi_pos_sup-1));
...
X_min=round(mid_Xaxis_large-0.51*size(scale_X,2));
X_max=round(mid_Xaxis_large+0.49*size(scale_X,2)-1);
...
for PROCESS_NUMBER=1:16
    ...
    psi_bin_bounds=1+radial_bin_size*(0:N_radial_bins-1)
    ...
    volume_radial_bin=zeros(N_radial_bins,1);
    ...
    for psi_rank=psi_pos_inf:psi_pos_sup+1
        for theta_rank=1:N_theta_bins
            ...
            volume_radial_bin(psi_rank)=Vtot_psi_rank;
    ...
    Npart_flux_Nradial=interp1((0:N_radial_bins-1)/(N_radial_bins-1),Npart_flux_surface,(0:Nradial-1)/(Nradial-1))
    
    density_part_ratio=density_on_axis*density_flux_surface(psi_pos_sup-5)/sum(Npart_theta_binned(psi_pos_sup-5,:),2)
    
    %last should not be in the counting for particles.....
    Npart_theta_binned(psi_pos_sup+1,:)=zeros(1,N_theta_bins);
    
    Nalphas_simulated=sum(sum(Npart_theta_binned));
    particles_weight=sum(Nalpha_binned(psi_pos_inf:psi_pos_sup).*volume_radial_bin(psi_pos_inf:psi_pos_sup))/sum(sum(Npart_theta_binned(psi_pos_inf:psi_pos_sup,:)))
    ...
    for psi_rank=psi_pos_inf:psi_pos_sup
        ...
        psi_bin_inf=psi_bin_bounds(psi_rank);
        psi_bin_sup=psi_bin_bounds(psi_rank+1);
        ...
        for(n=1:DNpart)
            psi_vector(n)=my_rand_linear_dist(Npart_flux_Nradial(psi_bin_inf),Npart_flux_Nradial(psi_bin_sup),psi_bin_inf,psi_bin_sup);
        end
        Nrank=1;
        for theta_rank=1:N_theta_bins
        ...
        for E_rank=1:N_energy_bins-1
            Nalpha_energy_rank(E_rank)=round(f_H_energy_bin_percentage(psi_rank,E_rank)*sum(Npart_theta_binned(psi_rank,:),2));
            for(n=Nrank:Nrank+Nalpha_energy_rank(E_rank)-1)
                ...
                Epart_vector(n)=my_rand_linear_dist(f_alpha_E_bin_edge_inf(psi_rank,E_rank),f_alpha_E_bin_edge_sup(psi_rank,E_rank),energy_H(E_rank)-0.5*energy_bin_size,energy_H(E_rank)+0.5*energy_bin_size);
...
%         pause
    ...
    compare_init_kinetic_TAE_profiles;
    particles_weight=particles_weight*density_on_axis/nTAE_initial_profile(PSI_BIN_SIZE)
%     density_part_ratio=density_part_ratio*density_on_axis/nTAE_initial_profile(PSI_BIN_SIZE+1)
    
    FILENAME=strcat('./initial_MB_H400_TAE_distribution',num2str(PROCESS_NUMBER),'.mat')
    save (FILENAME,'mHe','ZHe','particles_weight','alphas_pos_x','alphas_pos_z','alphas_pos_phi','alphas_Ekin','alphas_mm','alphas_vpll','Nalphas_simulated');
