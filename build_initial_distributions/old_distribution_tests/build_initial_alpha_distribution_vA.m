...
build_radial_alpha_particles_binning_vA;
...
for psi_rank=1:N_radial_bins
    for X_bin_rank=1:N_X_bins
        ...
        volume_bin(psi_rank,X_bin_rank)=sum(sum(volume_tor_diff(psi_pos_avg-delta_psi+1:psi_pos_avg+delta_psi,X_bin_pos_remap(X_bin_rank)-X_bin_half_std+1:X_bin_pos_remap(X_bin_rank)+X_bin_half_std),1),2);
    end
end
...
Btot_PR_map=sqrt(Btor_PR_map.^2+Bpol_PR_map.^2);

% Bavg=Btot_XZ_map(round(X_axis/DX)+mid_X ,round(Z_axis/DX)+mid_Z);
Bavg=mean(mean(Btot_PR_map))
average_alphas_temperature=(2/3)*mean(alphas_Ekin)*eV
average_alphas_beta=(BETA_ALPHAS/(2+BETA_ALPHAS))*plasma_beta_tot
average_Bfield_energy=(0.5*Bavg^2)*sum(sum(volume_tor_diff))/mu0
average_alphas_density=average_alphas_beta*average_Bfield_energy/average_alphas_temperature/sum(sum(volume_tor_diff))
frac_fusion=average_alphas_density/Ne0

density_part_ratio=frac_fusion*Ne0*density_flux_surface(N_radial_bins-2)/sum(Npart_binned(N_radial_bins-2,:),2)

% FILENAME=strcat(EQ_FOLDER,'initial_alphas_vA_distribution8.mat')
save (SAVENAME,'density_part_ratio','frac_fusion','alphas_pos_x','alphas_pos_z','alphas_pos_phi','alphas_Ekin','alphas_mm','alphas_vpll','Nalphas_simulated');
