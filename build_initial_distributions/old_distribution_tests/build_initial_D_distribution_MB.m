...
Npart_min=round(1.1*N_energy_bins)+1;
...
for psi_rank=1:N_radial_bins
    ...
    energy_rescale=0.5+0.5*Tvalues(psi_rank)/recalc_temperature(psi_rank)
    Epart_vector=Epart_vector*energy_rescale;
    
    % distribution in v_parallel range
    
    vpll_max=sqrt(2*(eV/mD)*Epart_vector);
    vpll_sup_values=sqrt(2*(Evalues+0.5*energy_bin_size)*eV/mD);
...
alphas_mm=(alphas_Ekin-0.5*(mD*alphas_vpll.^2)/eV)./interp2(scale_X,scale_Z,Btot_XZ_map',alphas_pos_x,alphas_pos_z);
...
save initial_D_MB_distribution2.mat density_part_ratio alphas_pos_x alphas_pos_z alphas_pos_phi alphas_Ekin alphas_mm alphas_vpll Nalphas_simulated mHe ZHe
