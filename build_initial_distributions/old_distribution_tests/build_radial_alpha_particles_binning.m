...
%     Ne0=4*10^19;
    ...
    lambda_D=sqrt(epsilon0*Te0*eV/(Ne0_psi*eV^2));
    Lambda_ei=9*(4*pi/3*Ne0_psi*lambda_D^3);
    
    log_lambda=log(Lambda_ei);
    
    tau_e=(3*(2*pi)^1.5)*(epsilon0^2*sqrt(me)*(eV*Te0)^1.5)/(Ne0_psi*log_lambda*eV^4);
    tau_eHe=(3*(2*pi)^1.5)*(epsilon0^2*sqrt(me)*(eV*Te0)^1.5)/(Ne0_psi*log_lambda*4*eV^4);
    
    tau_s=(Ni0*mDT/(Ne0_psi*me))*tau_eHe
    ...
    DeltaX=1/(NumberX-1);
	%Ti0 in keV
	averaged_cross_section=1.1*(1e-24)*(Ti0/1000)^2
    Salpha=0.25*Ni0*Ni0*averaged_cross_section;
    ...
    % beyond birth speed
    total_NumberX=ceil(1.2*v0_NumberX);
   ...
   for (x=v0_NumberX:total_NumberX)
    ...
    energy_bins_global=[1:energy_bin_size:round(1.88*NumberX)-1]';
    N_energy_bins=size(energy_bins_global,1);
    Evalues_global=E_norm(energy_bins_global);
    
    % 50 keV lower cutoff
    Erank_lower_cutoff=round(interp1(E_norm,1:size(E_norm,2), 150*1e3));
