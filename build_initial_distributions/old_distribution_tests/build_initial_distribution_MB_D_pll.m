...
build_radial_particles_binning_MB_D;

mHe=mD
ZHe=1


EDGE_ROTATION=0*1e3 % rad/s
rotation_profile=(Ne_profile/max(Ne_profile))*EDGE_ROTATION+EDGE_ROTATION;
...
Npart_simulated=22.0e3;
NB_PROCESS=1

% do not populate the edge
N_radial_bins=N_radial_bins-6;
psi_pos_sup=psi_pos_sup-6;
...
%     clear  alphas_pos_x alphas_pos_z alphas_pos_phi alphas_Ekin alphas_mm alphas_vpll
    ...
    for psi_rank=psi_pos_inf:psi_pos_sup
        for theta_rank=1:N_theta_bins
            ...
            volume_bin(psi_rank,theta_rank)=sum(sum(volume_dtheta_PR_map(theta_pos_inf:theta_pos_sup-1,psi_pos_avg-radial_bin_half_size:psi_pos_avg+radial_bin_half_size-1),1),2);
        end
    end
    ...
    for psi_rank=psi_pos_inf:psi_pos_sup
        ...
        plot(X_PR_map(:,psi_bin_inf),Z_PR_map(:,psi_bin_inf),'r');
        plot(X_PR_map(:,psi_bin_sup),Z_PR_map(:,psi_bin_sup),'r');
...
if NB_PROCESS>1
    for PROCESS_NUMBER=1:NB_PROCESS
        ...
        FILENAME=strcat(EQ_FOLDER,'initial_alphas_MB_D_distribution',num2str(PROCESS_NUMBER),'.mat')
        save (FILENAME,'mHe','ZHe','rotation_profile','density_part_ratio','alphas_pos_x','alphas_pos_z','alphas_pos_phi','alphas_Ekin','alphas_mm','alphas_vpll','alphas_momentum','Nalphas_simulated');
    ...
    Nalphas_simulated=Nalphas_simulated_tot
    
    FILENAME=strcat('initial_MB_D_distribution.mat')
    save (FILENAME,'mHe','ZHe','rotation_profile','density_part_ratio','alphas_pos_x','alphas_pos_z','alphas_pos_phi','alphas_Ekin','alphas_mm','alphas_vpll','alphas_momentum','Nalphas_simulated');
    vpll=alphas_vpll;
    input.m=mHe;
    input.Z=ZHe;
    input.Ekin=alphas_Ekin;
    clear x
    x(:,1)=alphas_pos_x;
    x(:,2)=alphas_pos_z;
    x(:,3)=alphas_pos_phi;
    save (FILENAME,'input','x','vpll');
