...
close all;
clear Evalues_norm f_alpha_energy_percentage_norm f_alpha_energy_percentage f_alpha_E_bin_edge_inf f_alpha_E_bin_edge_sup
clear f_alpha_vpll_percentage f_alpha_vpll_edge_inf f_alpha_E_bin_edge_sup
...
radial_bins=[radial_bin_half_size+1:radial_bin_size:simulation_size_r-1]';
...
%     Ne0=4*10^19;
    ...
    Ni0=(1/Zeff)*Ne0_psi;
    Te0=0.5*Pvalues(psi_pos)/Ne0_psi/eV  %eV
    Ti0=0.5*Pvalues(psi_pos)/((1/Zeff)*Ne0_psi)/eV; %eV
	vthe=sqrt(2*eV*Te0/me);
    vthi=sqrt(2*eV*Te0/mDT);
    
    
    lambda_D=sqrt(epsilon0*Te0*eV/(Ne0*eV^2));
    Lambda_ei=9*(4*pi/3*Ne0*lambda_D^3);
    
    log_lambda=log(Lambda_ei);
    
    tau_e=(3*(2*pi)^1.5)*(epsilon0^2*sqrt(me)*(eV*Te0)^1.5)/(Ne0*log_lambda*eV^4);
    tau_eHe=(3*(2*pi)^1.5)*(epsilon0^2*sqrt(me)*(eV*Te0)^1.5)/(Ne0*log_lambda*4*eV^4);
    
    tau_s=(Ni0*mDT/(Ne0*me))*tau_eHe;
...
%     w0=3.5*(10^6)*eV;
%     v0=sqrt(2*w0/mHe);
    w0=energy_upper_cutoff*(10^3)*eV;
    
    % approximative parameters
    vc=sqrt(44*2*Te0*eV/mHe);
    v0=vc;

    
    DeltaX=1/(NumberX-1);
	%Ti0 in keV
	averaged_crosse_section=1.1*(1e-24)*(Ti0/1000)^2
    Salpha=0.25*Ni0*Ni0*2*averaged_crosse_section;
    ...
    for (x=0.25*NumberX:2*NumberX-1)
        nubi(x)=(1/(4*pi))*(4*Ni0*log_lambda*eV^4)/((epsilon0^2)*mr*mHe)*(1/(vb(x)^3+1.3*vthi^3));
        nube(x)=(1/(4*pi))*(4*Ne0*log_lambda*eV^4)/((epsilon0^2)*me*mHe)*(1/(vb(x)^3+1.3*vthe^3));
    end
    ...
    for (x=1000:2*NumberX-1)
        nubi(x)=(1/(4*pi))*(4*Ni0*log_lambda*eV^4)/((epsilon0^2)*mr*mHe)*(1/(vbc(x)^3+1.3*vthi^3));
        nube(x)=(1/(4*pi))*(4*Ne0*log_lambda*eV^4)/((epsilon0^2)*me*mHe)*(1/(vbc(x)^3+1.3*vthe^3));
    end
    for (x=1:1000)
        nubi(x)=nubi(1000);
        nube(x)=nube(1000);
    ...
    % beyond birth speed
    total_NumberX=ceil(2.2*v0_NumberX);
...
%         phi_s(x)=(2*mHe/mDT)*(1+speed_range(x)^3);
%         w(x)=0.5*mHe*vc^2*speed_range(x)^2;
%         f_alpha(x)=exp(-1/(Te0*eV)*(w(x)-w0))*(s_var)/(phi_s(x));
%     end
    ...
    NumberX_norm=round(0.5*NumberX)-1;
    % rescaling the distribution to have equal energy bins
    f_alpha_norm=interp1((1:total_NumberX)*NumberX_norm/(total_NumberX-1),f_alpha,1:NumberX_norm);
    v_norm=interp1(((1:total_NumberX))*NumberX_norm/(total_NumberX-1),vbc,1:NumberX_norm);
    v_norm(1)=0;
    E_norm=(0.5*mHe/eV)*v_norm.^2;
    
    % binning in total kinetic energy values
    energy_bins_global=[1:energy_bin_size:round(0.5*NumberX)-1]';
    N_energy_bins=140;
    Evalues_global=E_norm(energy_bins_global);
    
    % 50 keV lower cutoff
    % 8 keV lower cutoff
    Erank_lower_cutoff=round(interp1(E_norm,1:size(E_norm,2), 6*1e3));
