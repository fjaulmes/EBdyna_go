...
radial_bins=[radial_bin_half_size+1:radial_bin_size:simulation_size_r-58]';
...
energy_bin_size=0.5*1e3; %keV
energy_max=32*1e3; %keV
...
for psi_pos=1:N_radial_bins
    ...
    Te_value=0.5*(Pvalues(psi_pos)./Nvalues(psi_pos))/eV;  %eV
%     Te_value=Te0;
    Ti_value=Te_value; %eV     % Ti=Te
    Tvalues(psi_pos)=Ti_value;
	vthe=sqrt(2*eV*Te_value/me);
    vthi=sqrt(2*eV*Ti_value/mD);
    ...
    for n=1:N_energy_bins+1
        f_DT_speed_dist(psi_pos,n)=((mD/(2*pi*Ti_value*eV))^(3/2))*(4*pi*speed_DT_range(n)^2)*exp(-(mD*speed_DT_range(n)^2)/(2*Ti_value*eV));
        f_DT_E_dist(psi_pos,n)=(2*sqrt(energy_DT_range(n)*eV/pi).*(1/mD*Ti_value*eV)^(3/2)).*exp(-energy_DT_range(n)/Ti_value);
    end
