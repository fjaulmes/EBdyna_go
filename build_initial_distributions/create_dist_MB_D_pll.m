...
volume_flux=cumsum(sum(dVol,1));

Npart_simulated=2.6e6;
...
volume_bin_radial_edges=interp1(2:Nradial,volume_flux,radial_bins_edges);
...
%     clear  alphas_pos_x alphas_pos_z alphas_pos_phi alphas_Ekin alphas_mm alphas_vpll
    ...
    for psi_rank=psi_pos_inf:psi_pos_sup
        ...
        psi_bin_inf=radial_bins_edges(psi_rank);
        psi_bin_sup=radial_bins_edges(psi_rank+1);
...
%         
    ...
    alphas_pos_x=alphas_pos_x(find(~ejected));
    alphas_pos_z=alphas_pos_z(find(~ejected));
    alphas_pos_phi=alphas_pos_phi(find(~ejected));
    alphas_Ekin=alphas_Ekin(find(~ejected));
    alphas_mm=alphas_mm(find(~ejected));
    alphas_vpll=alphas_vpll(find(~ejected));
...
% averaging rotation effects to check confinement
%     alphas_momentum=alphas_pos_psi*0+0*CORE_ROTATION;
    ...
    nb_part_values=density_pp(1:length(psi_edges)-1)*0;
    volume_edge_bins=interp1(psi_scale(2:end),volume_flux,psi_edges);
    volume_bins=volume_edge_bins(2:end)-volume_edge_bins(1:end-1);
    
    psi_middle=interp1(1:NB_BINS+1,psi_edges,round(NB_BINS/2));
    density_ref=interp1(psi_scale,D_density_profile,psi_middle);
    ...
    FILENAME=strcat('initial_alphas_MB_D_distribution',num2str(round(Nalphas_simulated/1e3)),'k.mat')
    ...
    figure;hold on
    grid on
    plot(psi_scale,D_density_profile,'r')
