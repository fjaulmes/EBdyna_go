...
for i=2:(size(sigma,1)-1)
    ...
    %I(i,:)=I(i-1,:)-dI(i-1,:);
    %RK4
    k1(i-1,:)=ne(i,:).*sigma(i,:).*I(i-1,:).*1e-4;
    k2(i-1,:)=0.5*ne(i,:).*(sigma(i,:)+sigma(i+1,:)).*(I(i-1,:)+dl./2.*k1(i-1,:)).*1e-4;
    k3(i-1,:)=0.5*ne(i,:).*(sigma(i,:)+sigma(i+1,:)).*(I(i-1,:)+dl./2.*k2(i-1,:)).*1e-4;
    k4(i-1,:)=ne(i,:).*sigma(i,:).*(I(i-1,:)+dl.*k3(i-1,:)).*1e-4;    
    dI(i-1,:)=dl./6.*(k1(i-1,:)+2.*k2(i-1,:)+2.*k3(i-1,:)+k4(i-1,:));
    I(i,:)=I(i-1,:)-dI(i-1,:);
end
