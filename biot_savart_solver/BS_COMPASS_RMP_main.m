function [ ] = BS_COMPASS_RMP_main(PROCESS_NUMBER,N_PROCESS)
...
[RMP_map,Z_correction]=BS_COMPASS_get_grid(RMP_map);

%% Calculate the fields of each coil
[RMP_map] = BS_COMPASS_RMP_calc_coils_individual(par,RMP_map,RMP_coils,Z_correction);
...
%% Save output
if par.example==0
    save_name_individual=strcat('./output/BS_COMPASS_RMP_',par.coord_sys,'_individual_',datestr(now,'yyyy-mm-dd'),'_process',num2str(par.PROCESS_NUMBER),'.mat');
...
%     disp(strcat('Saved n=',num2str(tor_mode),' file: ',num2str(par.PROCESS_NUMBER),' of ',num2str(par.N_PROCESS)))
    ...
    tor_mode=2;
    parity_name='even';
    RMP_coils=BS_COMPASS_RMP_add_signs(RMP_coils,tor_mode,parity_name);
    [ RMP_coils,field] = BS_COMPASS_RMP_apply_mode(RMP_map,RMP_coils);
...
switch par.example
    ...
    case 2
        ...
        hf=figure('tag','BS_COMPASS_RMP_map_RMP_coils_figure');
        ha1=axes('parent',hf);
...
for i=1:length(Au)
    h=plot3(ha1,Au(i).X,Au(i).Y,Au(i).Z,'k','displayname',['Au coil ',num2str(i)]);
    if Au(i).ref_coil==3
        set(h,'color','g')
    end
    if Au(i).sign==1
        text(mean(Au(i).X),mean(Au(i).Y),mean(Au(i).Z),'\textbf{+}','color',get(h,'color'),'interpreter','latex','parent',ha1,'FontSize',20)
    elseif A(i).sign==-1
        text(mean(Au(i).X),mean(Au(i).Y),mean(Au(i).Z),'\textbf{-}','color',get(h,'color'),'interpreter','latex','parent',ha1,'FontSize',20)
    end
    set(h,'linewidth',3)
...
for i=1:length(Al)
    h=plot3(ha1,Al(i).X,Al(i).Y,Al(i).Z,'k','displayname',['Al coil ',num2str(i)]);
    if Al(i).ref_coil==3
        set(h,'color','g')
    end
    if Al(i).sign==1
        text(mean(Al(i).X),mean(Al(i).Y),mean(Al(i).Z),'\textbf{+}','color',get(h,'color'),'interpreter','latex','parent',ha1,'FontSize',20)
    elseif A(i).sign==-1
        text(mean(Al(i).X),mean(Al(i).Y),mean(Al(i).Z),'\textbf{-}','color',get(h,'color'),'interpreter','latex','parent',ha1,'FontSize',20)
    end
    set(h,'linewidth',3)
