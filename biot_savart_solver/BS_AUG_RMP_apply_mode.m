function [ RMP_coils ,field] = BS_AUG_RMP_apply_mode(RMP_map,RMP_coils )
...
if nargin==0
    ...
    for i=1:length(fnames)
        ...
        [ ~,field] = BS_AUG_RMP_apply_mode(loaded_data.RMP_map,loaded_data.RMP_coils,tor_mode,parity);
        par=loaded_data.par;
        field=remove_fields(field,remove);
        save_name_even2=strcat('./output/BS_AUG_RMP_n=',num2str(tor_mode),'_even_',par.coord_sys,'field_',datestr(now,'yyyy-mm-dd'),'_processmode',num2str(par.PROCESS_NUMBER),'.mat');
