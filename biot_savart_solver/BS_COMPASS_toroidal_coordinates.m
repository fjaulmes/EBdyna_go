...
if ~exist('z_axis')
	warning('Using data file to read Z-offset. Based on METIS simulation!!!')
	filename='../METIS_profiles.mat';
	if exist(filename,'file')
		load(filename,'METISgeo');        
		% METISgeo=METISgeo.METISgeo;
		z_axis=METISgeo.Zaxis;         % position magnetic axis (m)
		T_data=METISgeo.time_METIS;         % time
		time_eq=0.0;                            % time equilibrium 
