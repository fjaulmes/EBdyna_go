...
if ~exist('z_axis') || (z_axis==0)
	warning('Using data file to read Z-axis pos. Based on FIESTA simulation!!!')
	filename='../data_tokamak/FIESTA_equil.mat';
	if ~exist(filename,'file')
	filename='../FIESTA_equil.mat';
	end
	if exist(filename,'file')
		load(filename,'FIESTA');        
		% METISgeo=METISgeo.METISgeo;
		z_axis=FIESTA.params.z0_geom;         % position magnetic axis (m)
		T_data=FIESTA.time_equil;         % time
		time_eq=1.55;                            % time equilibrium 
