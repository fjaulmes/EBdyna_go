...
switch nargin
    case 0
        ...
        TF=load(['./output_TF/TF_',coord_syst,'_',datestr(now,'yyyy-mm-dd'),'.mat']);
    case 1
        coord_syst='toroidal';
        warning(['Assuming a ',coord_syst,' coordinate system!'])
    case 2
...
for BA=BA_fields
    % keep BZ as total field for 3D turn-to-turn perturbation
...
if strcmp(coord_syst,'flux')
    ...
    sca_fac=mean(sca_fac);
        ...
        warning('Switching to no rescaling....')
        sca_fac=1;
%        sca_fac=5.0/3.8
    end
end
...
TFR=TF;
for BA=BA_fields
    % added BR and BZ and Aphi in fields excluded from average
    if any(strcmp(BA{1},{'BR','BZ','Aphi','dAR_dphi','dAZ_dphi','dAphi_dphi'})); continue; end; % One doesn't need a 2D-variant of phi-derivatives (=0), but it needs to be scaled
	TFR.(BA{1})=bsxfun(@minus,TF.(BA{1}),TF_2D.(BA{1}));
end

%% Save output
save(['./output_TF/TFR_',coord_syst,'_',datestr(now,'yyyy-mm-dd'),'.mat'],'-v7.3','-struct','TFR');
