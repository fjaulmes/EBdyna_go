function [ n3D_map,Z_correction ] = BS_COMPASS_get_grid( n3D_map )
...
%                 Z_correction=double(data_tokamak.z_axis)-dim.Z_axis;
            ...
            case 'toroidal'
                ...
                nr_X=1.0*numel(dim.scale_X);
                nr_Z=1.0*numel(dim.scale_Z);
                % modified for 16 TF coils 3D coils
                nr_phi=321;
                ...
                [n3D_map,data_tokamak]=BS_COMPASS_toroidal_coordinates(n3D_map);    % Determine with flux coordinates (psi,theta,phi)
