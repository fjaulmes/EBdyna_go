...
if ~isfield(output,'ndd_val')
    output.ndd_val=output.Ekin*0;
end
if ~isfield(output,'ndd_val_thermal')
    output.ndd_val_thermal=output.ndd_val*0;
end
if ~isfield(output,'ndd_val_BB')
...
if isfield(output,'ejected_CX_dt')&par.CALCULATE_CX
    ...
    Rbins = [Rbins(1)-(Rbins(2)-Rbins(1)) Rbins Rbins(end)+(Rbins(2)-Rbins(1)) Rbins(end)+2*(Rbins(2)-Rbins(1)) Rbins(end)+3*(Rbins(2)-Rbins(1)) Rbins(end)+4*(Rbins(2)-Rbins(1))];
    Zbins = scale_Z(1:30:end);
    Rvals = Rbins(1:end-1)+0.5*(Rbins(2)-Rbins(1));
    Zvals = Zbins(1:end-1)+0.5*(Zbins(2)-Zbins(1));
