...
function summary_COMPASS_NBI(FNAME,SAVE_NEW_PREC_INPUT)
...
try
    load('../../../data_common/COMPASS_vessel_limits.mat')
catch
    load('../data_common/COMPASS_vessel_limits.mat')
end
...
%security
ejected(output.Ekin(:,end)>2e5)=1;
output.Ekin(output.Ekin>2e5)=0;
...
if isfield(output,'ndd_val')
    ...
    dEkin_values=squeeze(output.Ekin(~output.ejected_wall_dt,end)-output.Ekin(~output.ejected_wall_dt,1));
    
    Rbins = scale_X(1:14:end)+R0;
    Zbins = scale_Z(1:14:end);
    ...
    power_map=zeros(length(Rvals),length(Zvals));
    ...
    for Rindex=1:length(Rvals)
        for Zindex=1:length(Zvals)
            ...
            power_map(Rindex,Zindex)=sum(abs(dEkin_values(POP_BIN)));
        end
    end
    % ndd_map=griddata(Rpos,Zpos,ndd_values,Zmesh,Rmesh);
    power_map=power_map.*(par.MARKER_WEIGHT*eV/DT_SIM_RECORD);
    
    summary.Rvals_power=Rvals;
    summary.Zvals_power=Zvals;
    summary.DV_map_power=DV_map;
    summary.power_map=power_map./DV_map;
    
    if PLOT_2D
        figure
        hold on
        imagesc(Rvals,Zvals,(power_map./DV_map)')
        plot_COMPASS_vessel;
        ylim([-0.4 0.4])
        xlim([0.32 0.76])
...
R_OWL_UP=max(R_outer_vessel);
Z_OWL_UP=max(Z_outer_vessel);
R_OWL_DOWN=min(R_outer_vessel);
Z_OWL_DOWN=min(Z_outer_vessel);
...
%             CX_power_map_2D(Zindex,Rindex)=sum(mean(abs(output.Ekin(POP_BIN,:)),2)); % total power gone in to CX
    ...
    plot_COMPASS_vessel;
    set(gca,'fontsize',20)
    title('NBI neutralized to CX [W/m^{3}]','fontsize',20)
    ylim([-0.62 0.6])
    xlim([0.55 1.22])
    ...
    hold on
    grid on
    plot(Rbins_scale,mean(CX_Ekin_power_map_2D,1),'linewidth',3);
    plot(Rbins_scale,mean(neutrals_power_map_2D,1),'linestyle','-.','linewidth',3);
    xlabel('R [m]')
    ylabel('[W/m^{3}]')
    xlim([0.56 1.23])
...
if isfield(output,'ndd_val')
    ...
    Rbins = scale_X(1:10:end)+R0;
    Zbins = scale_Z(1:10:end);
    Rvals = Rbins(1:end-1)+0.5*(Rbins(2)-Rbins(1));
    Zvals = Zbins(1:end-1)+0.5*(Zbins(2)-Zbins(1));
...
if isfield(output,'ejected_CX_dt')&par.CALCULATE_CX
    ...
    if PLOT_CXLOSS
        ...
        set(gca,'fontsize',20)
        title('COMPASS NBI-CX (1MW) W/m^{2} ')
...
%     summary.DV_map_cxloss=DV_map;
    ...
    if PLOT_IONLOSS
        ...
        set(gca,'fontsize',20)
        title('CU NBI-ions-losses (1MW) W/m^{2} ')
