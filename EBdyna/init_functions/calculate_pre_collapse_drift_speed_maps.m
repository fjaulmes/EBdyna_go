function []=calculate_pre_collapse_drift_speed_maps(ISSQUAREMAP,SOL_MAX_PSI_NORM1,ADJUSTX1,ADJUSTX2,ADJUSTZ1,ADJUSTZ2)

initialize_folder_names;    %Find the folders and load:

if nargin<2
    SOL_MAX_PSI_NORM1=1.2;
...
if ISSQUAREMAP
    ...
    minX=max(mid_X_large-round(ADJUSTX1*Nradial),2);
    maxX=min(mid_X_large+round(ADJUSTX2*Nradial),SIZEX_LARGE-1);
    minZ=max(mid_Z-round(ADJUSTZ1*elongation*Nradial),1);
    maxZ=min(mid_Z+round(ADJUSTZ2*elongation*Nradial),SIZEZ_LARGE-1);
...
%LSN configuration
if FIESTA.Z_xpoint<max(FIESTA.Z_boundary)
    ...
    psi_norm1_XZsmall_map((psi_norm1_XZsmall_map<1)&(Zpos_map>max(FIESTA.Z_boundary)))=2-psi_norm1_XZsmall_map((psi_norm1_XZsmall_map<1)&(Zpos_map>max(FIESTA.Z_boundary)));
else
    %USN configuration
    psi_norm1_XZsmall_map((psi_norm1_XZsmall_map<1)&(Zpos_map>FIESTA.Z_xpoint))=2-psi_norm1_XZsmall_map((psi_norm1_XZsmall_map<1)&(Zpos_map>FIESTA.Z_xpoint));
    psi_norm1_XZsmall_map((psi_norm1_XZsmall_map<1)&(Zpos_map<min(FIESTA.Z_boundary)))=2-psi_norm1_XZsmall_map((psi_norm1_XZsmall_map<1)&(Zpos_map<min(FIESTA.Z_boundary)));
end
...
scale_R_mp=scale_X(mid_Xaxis+1:length(scale_X)-1)+R0;
