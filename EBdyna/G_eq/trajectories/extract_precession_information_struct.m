...
% need to overwrite the previous psi_norm to have gc values
...
if ~isfield(dim,'radial_r_value_flux')
    ...
    r=interp1(1:length(dim2.radial_r_value_flux),dim2.radial_r_value_flux,output.psi_norm,'*pchip');
else
    r=interp1(1:length(dim.radial_r_value_flux),dim.radial_r_value_flux,output.psi_norm,'*pchip');
end

% Find r-value : avg and min, max values [from gc pos]
r_avg=mean(r,2);
r_max=max(r');
r_min=min(r');
%orbit width in meters
% prec.r_min=r_min;
% prec.r_max=r_max;
prec.delta_r=(r_max-r_min)';
...
for i=1:N_job
    theta_prev(i)=output.theta_gc(i,min(HFS_theta_ind(i)  ,size(output.theta_gc,2)))-output.theta_gc(i,max(HFS_theta_ind(i)-8,1));
    theta_next(i)=output.theta_gc(i,min(HFS_theta_ind(i)+8,size(output.theta_gc,2)))-output.theta_gc(i,max(HFS_theta_ind(i)  ,1));
end
minX=min(output.x_gc(:,1,:),[],3)-dim.R0;
maxX=max(output.x_gc(:,1,:),[],3)-dim.R0;

% two conditions need to be verified
HFS_crossing=sign(theta_prev)==sign(theta_next);
HFS_crossing=(minX<=dim.X_axis) & HFS_crossing;

HFS_crossing=(HFS_crossing & ((HFS_theta<=(par.TOL_CORE_HFS_THETA-(par.TOL_CORE_HFS_THETA-par.TOL_OUTER_HFS_THETA)*sqrt(r_avg/max(r_avg))))));
...
prec.pop.ALL_PASSING=       ~sign_change &   HFS_crossing &  sigma_LFS           ...
                     |(~sign_change  &  sigma_LFS &  sigma_HFS);
...
prec.pop.STAGNATION=       (~sign_change &    sigma_LFS &    ~sigma_HFS)...
                     |(~sign_change &  ~sigma_LFS)          ;

prec.pop.CO_PASSING=       (~sign_change &	sigma_HFS &  sigma_LFS &                     (sign_vpll== 1))...
                |     (~sign_change  &  sigma_LFS &     sigma_HFS &     (sign_vpll== 1));

prec.pop.COUNTER_PASSING=  (~sign_change &   sigma_HFS &  sigma_LFS &                     (sign_vpll==-1))...
                |     (~sign_change  &  sigma_LFS &     sigma_HFS &     (sign_vpll==-1));
...
DPPHI=(abs(min(output.pphi_kin(:,:)')-max(output.pphi_kin(:,:)')))';
prec.pop.SMALL_DPPHI=(DPPHI<=par.TOL_PREC_DPPHI);
...
prec.vpll_avg=mean(output.vpll,2);
