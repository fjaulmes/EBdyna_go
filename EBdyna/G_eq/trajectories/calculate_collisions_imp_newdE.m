function [ v Eexch_e  Eexch_i Eexch_imp  Etot_e  Etot_i Etot_imp delta_omega] = calculate_collisions_imp(const,par,dim,X_SLOW_ELEC_THRESH,input,...
    Ti,vth_i,ni,psi_gc_value,psi_xi_pos,psi_slope,v_norm,Ekin,v,B,Bfield,Eexch_e,Eexch_i,Eexch_imp,Etot_e,Etot_i,Etot_imp,TESTING_SD)
...
vth_e=sqrt(2*Te*(const.eV/const.me));
% vth_i=sqrt(2*Ti*(const.eV/const.mBulk));
vth_imp=sqrt(2*Ti*(const.eV/par.mimp));

% modified from 2*Te to 3*Te in order to match ASCOT data
% inconsistent with most derivations (Freidberg, Callens ...)

% veff_e=sqrt(2*Te.*const.eV./const.me);
...
nu_imp=(1/(4*pi)).*(par.IMPURITY_ZAVG.*input.Z).^2.*(const.eV^2/const.epsilon0)^2./(input.m.^2).*nimp;

% log(lambda) : original EBdyna paper, modified with new effective velocity for ions

% veff_e=max(sqrt(2*Te.*const.eV./const.me),v_norm);
veff_e=sqrt(2*Te.*const.eV./const.me+v_norm.^2);
log_lambda_Fe=15.2-0.5*log(ne*1e-20)+log(0.5.*1e-3.*(const.me/const.eV).*veff_e.^2);
...
veff_i_sq1=(Ti.*const.eV./const.mBulk+v_norm.^2);
veff_imp_sq1=(Ti.*const.eV./par.mimp+v_norm.^2);
...
omega_cimp_sq=1.74.*(par.IMPURITY_ZAVG^2./par.Aimp).*nimp+(9.163e15/par.Aimp^2).*Bfield.^2;
rmax=((omega_ce_sq./veff_e_sq1)+(omega_ci_sq./veff_i_sq1)+(omega_cimp_sq./veff_imp_sq1)).^-0.5;

% veff_e_sq2=(3.*Te.*const.eV./const.me+v_norm.^2);
veff_i_sq2=(3.*Ti.*const.eV./const.mBulk+v_norm.^2);
veff_imp_sq2=(3.*Ti.*const.eV./par.mimp+v_norm.^2);

rmin_cl_imp=0.13793.*(input.Z).*(par.IMPURITY_ZAVG).*(par.Aimp+input.Afast)./(input.Afast.*par.Aimp.*veff_imp_sq2);
...
rmin_qu_imp=1.9121e-8.*(par.Aimp+input.Afast)./(input.Afast.*par.Aimp.*sqrt(veff_imp_sq2));
...
rmin_imp=max(rmin_cl_imp,rmin_qu_imp);
rmin_Bulk=max(rmin_cl_Bulk,rmin_qu_Bulk);
% rmin_e=max(rmin_cl_e,rmin_qu_e);

log_lambda_Fimp=log(rmax./rmin_imp);
...
nu_imp=nu_imp.*log_lambda_Fimp./(v_norm.^3);
...
pmpp_x=(input.m./const.mBulk).*psi_x-psi_prime_x; % factor 2 remove as we consider velocity evolution (not energy)
...
% finally considering impurity ion contribution


sqrt_x_v=(v_norm./vth_imp);
x_v=sqrt_x_v.^2;
FAST_ION_GROUP=x_v>X_SLOW_ELEC_THRESH;
psi_x(FAST_ION_GROUP)=1-(2*(sqrt_x_v(FAST_ION_GROUP)).*exp(-x_v(FAST_ION_GROUP))./sqrt(pi)).*(1+1./(2.*x_v(FAST_ION_GROUP))-1./(4.*x_v(FAST_ION_GROUP).^2));
psi_x(~FAST_ION_GROUP)=0.752252778.*(x_v(~FAST_ION_GROUP).^1.5).*(1-3.*x_v(~FAST_ION_GROUP)/5+3.*x_v(~FAST_ION_GROUP).^2/14);
psi_prime_x=(2*(sqrt_x_v).*exp(-x_v)./sqrt(pi));
pmpp_x=(input.m./par.mimp).*psi_x-psi_prime_x; % factor 2 remove as we consider velocity evolution (not energy)

nu_perp=nu_perp+nu_imp.*((psi_x+psi_prime_x-psi_x./(2.*x_v))); 

nu_imp=nu_imp.*pmpp_x;
...
if par.CALCULATE_PDEP
    if par.SLOWING_DOWN
        ...
        dv_imp=par.NR_FUND_IN_LOOP.*par.dt.*v_norm.*nu_imp;
        dv_e=max(dv_e,-V_FRAC_LOSS_MAX.*v_norm);
        dv_i=max(dv_i,-V_FRAC_LOSS_MAX.*v_norm);
    end
    delta_v=dv_e+dv_i+dv_imp;
	% dE_tot=0.5*(input.m/const.eV).*(2.*v_norm.*delta_v-delta_v.^2);
    % energy lost is given to plasma species
    if par.SLOWING_DOWN
        DE_e=0.5*(input.m/const.eV).*(2.*(v_norm.^2).*nu_e).*par.NR_FUND_IN_LOOP.*par.dt;
        DE_i=0.5*(input.m/const.eV).*(2.*(v_norm.^2).*nu_i).*par.NR_FUND_IN_LOOP.*par.dt;
        DE_imp=0.5*(input.m/const.eV).*(2.*(v_norm.^2).*nu_imp).*par.NR_FUND_IN_LOOP.*par.dt;
        %DE_e=0.5*(input.m/const.eV).*(2.*v_norm.*dv_e-dv_e.^2);
        %DE_i=0.5*(input.m/const.eV).*(2.*v_norm.*dv_i-dv_i.^2);
        %DE_imp=0.5*(input.m/const.eV).*(2.*v_norm.*dv_imp-dv_imp.^2);
...
%         dv_i=max(dv_i,-EKIN_FRAC_LOSS_MAX.*Ekin);
    ...
    Eexch_e=Eexch_e+DE_e;
    Eexch_i=Eexch_i+DE_i;
    Eexch_imp=Eexch_imp+DE_imp;
    Etot_e=Etot_e+DE_e;
    Etot_i=Etot_i+DE_i;
    Etot_imp=Etot_imp+DE_imp;
else
    nu_tot=nu_e+nu_i+nu_imp;
    if par.SLOWING_DOWN
