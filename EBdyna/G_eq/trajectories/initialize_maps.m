...
if strcmp(mach,'SALOMON')
	disp('No shared memory for runs on Salomon!');
	load_worker=false;
	maker=true;
end
...
if par.COULOMB_COLL || par.CALCULATE_NDD || par.CALCULATE_CX
    filename=[par.paths.DATA_FOLDER,'pressure_profile.mat'];
%     d2=load(filename,'psi_norm','vtor_prof','n0_prof','ni_prof','Ti_prof','Te_prof','nuNBie','nuNBii_v3','vthi_prof','vthe_prof');
    ...
    if size(d2.vtor_prof,2)>size(d2.vtor_prof,1)        d2.vtor_prof=d2.vtor_prof'; end
    if size(d2.ni_prof,2)>size(d2.ni_prof,1)        d2.ni_prof=d2.ni_prof'; end    
    ...
    if isfield(par,'REMOVE_SOL_PLASMA')
        if par.REMOVE_SOL_PLASMA
            ...
            d2.ne_prof(d2.psi_norm>1)=0;
            d2.ni_prof(d2.psi_norm>1)=0;
        end
    else
        par.REMOVE_SOL_PLASMA=0;
    end
	% create simplified radial scale for faster nDD Beam-Beam calculations
	if isfield(par,'RECORD_VSPACE_NDD')
		if par.RECORD_VSPACE_NDD
			d2.pn_nDD_bins=linspace(d2.psi_norm(1),d2.psi_norm(end),round(length(d2.psi_norm)/3)+1);
		end
	else
		par.RECORD_VSPACE_NDD=0;
	end
    d=combine_structs(d,d2);
    % risky: generated by previous run!!!
    if isfield (par,'RECORD_VSPACE_NDD')
        if par.RECORD_VSPACE_NDD
		    try
				d2=load(filename,'nfast_prof');
    ...
    % needs impurity profile to consider impurities
    % otherwise need parameter for impurity fraction of ni
    if isfield (par,'IMPURITY_ZAVG')
        if par.IMPURITY_ZAVG>0
            d2=load(filename,'nimp_prof');
            if ~isempty(d2)
                if size(d2.nimp_prof,2)>size(d2.nimp_prof,1)        d2.nimp_prof=d2.nimp_prof'; end
            else
    ...
    if isfield (par,'CALCULATE_NDD_THERMAL')
        if par.CALCULATE_NDD_THERMAL
		    try
            d3=load(filename,'DV_prof');
            if ~isempty(d3)
                if size(d3.DV_prof,2)>size(d3.DV_prof,1)        d3.DV_prof=d3.DV_prof'; end
            else
                % dummy volume profile to get something proportional to
                % density (weight / m^3)
                d3.DV_prof=d.psi_norm*0+1;
            end
...
%   rescaling the profiles to make a scan in density
    ...
    filename=[par.paths.DATA_COMMON,'sigma_cxD_T0_log.mat'];
    d2=load(filename, 'Eb_values_log_lin', 'Elog_scaling', 'Emin_eV', 'T0_values_log_lin', 'T0log_scaling', 'T0min_eV', 'sigma_cxD_T0_loglog', 'sigma_ionD_Ti_loglog');
    d2.DElog_inv=1/(d2.Eb_values_log_lin(2)-d2.Eb_values_log_lin(1));
    d2.DT0_inv=1/(d2.T0_values_log_lin(2)-d2.T0_values_log_lin(1));
    d=combine_structs(d,d2);
end

if par.mode==5 || par.APPLY_SAWTOOTH
    filename=[par.paths.DATA_FOLDER,'q_profile.mat'];
    d2=load(filename,'q_initial_profile'); 
    d=combine_structs(d,d2);
end
...
if par.USE_VESSEL_LIMIT
     filename=[par.paths.DATA_FOLDER,par.VESSEL_FILENAME]
