function [m,d] = initialize_maps_ram
...
if isempty(const)
    filename=[par.folders.DATA_COMMON_PHYSICS,'physics_constants.mat'];
    const=load(filename);
end
...
if par.mode==5 || par.APPLY_SAWTOOTH
    filename=[par.folders.DATA_SHOT,'q_profile.mat'];
...
fnames=fieldnames(m)';
for i=1:length(fnames)
    if size(m.(fnames{i}),2)~=d.size_Z
        m.(fnames{i})=m.(fnames{i})';
    end
end
