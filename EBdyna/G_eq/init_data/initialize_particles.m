...
elseif isfield(dist,'Nalphas_simulated')|isfield(dist,'FI_NBI')
...
elseif isfield(dist,'input')
    ...
    input=dist.input;
    input.Z=double(input.Z); % to avoid undesired converion to int64
...
if ~isfield(par,'MARKER_WEIGHT')    
	...
	%disp(['length (input.Ekin) = ' num2str(length (input.Ekin))]);
...
qom=double(input.Z)*const.eV/input.m;       %Kinectic constant
...
if isfield (par,'CALCULATE_NDD_THERMAL')
    if par.CALCULATE_NDD_THERMAL
        INCREASE_STATS_NI=2; % for more realistic distribution of thermal markers
        dim.LENGTH_NI_STATS=input.N_total*INCREASE_STATS_NI;
        dim.x_ni=nan(dim.LENGTH_NI_STATS,3);
%         dim.x_ni_cyl=nan(input.N_total*2,3);
        dim.ni_val=zeros(dim.LENGTH_NI_STATS,1);
        dim.ni_markers_weight=zeros(dim.LENGTH_NI_STATS,1);
        dim.Ti_x_ni=zeros(dim.LENGTH_NI_STATS,1);
        [dim.x_ni dim.Ti_x_ni dim.ni_val dim.ni_markers_weight]=generate_radial_ni_markers_for_nDDth(dim,maps,input);
        dim.ni_markers_weight=dim.ni_markers_weight.*(dim.LENGTH_NI_STATS/input.N_job);
    end
else
...
if isfield(par,'RECORD_LOSS_TRAJECTORY')
...
if par.CALCULATE_NDD
    ...
    if par.RECORD_VSPACE_NDD
        output.ndd_val_BP=zeros(input.N_job,par.NB_TIME_STAMPS);
        output.ndd_val_BB=zeros(input.N_job,par.NB_TIME_STAMPS);
        output.v_n_BP=NaN([size(x),par.NB_TIME_STAMPS]);
        output.v_n_BB=NaN([size(x),par.NB_TIME_STAMPS]);
    end
    if par.CALCULATE_NDD_THERMAL
	  output.ndd_val_thermal=zeros(input.N_job,par.NB_TIME_STAMPS);
      output.v_n_thermal=NaN([size(x),par.NB_TIME_STAMPS]);
      output.x_n_thermal=NaN([size(x),par.NB_TIME_STAMPS]);
    end
end
...
if par.IMPURITY_ZAVG>0
    output.Pdep_imp=zeros(input.N_job,par.NB_TIME_STAMPS);
    output.Etot_imp=zeros(input.N_job,par.NB_TIME_STAMPS);
end
if par.COULOMB_COLL
