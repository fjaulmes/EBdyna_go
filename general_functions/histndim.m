function [jda,kda,vda,v,t,handle]=histndim(x,b,mode,varargin)
...
%                         B(1,:) gives the number of bins in each dimension [default 10]
...
% parent
prnt=find(strcmpi(varargin,'parent'));
if isempty(prnt); prnt = gca; else; prnt=varargin{prnt+1}; end
% displayname
dpname=find(strcmpi(varargin,'displayname'));
if isempty(dpname); dpname = 'data1'; else; dpname=varargin{dpname+1}; end
%% Body basis
...
if length(svg)==1
    ...
    error('Unable to retrieve jda and kda in this function')
elseif length(svg)==2
    j=acd(svg(1));
    k=acd(svg(2));
    bj=b(1,j);
    ...
    if ~nargout || any(mode=='h')
        handle=surf(jda,kda,vda,'parent',prnt,'displayname',dpname);
    end
else
